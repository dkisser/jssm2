...
<html>
  ...
  <body>
  	<style type="text/css">
   	...
   	<div class="easyui-layout" style="text-align: center;" data-options="fit:true">
				...
				$.messager.confirm('确认上传', '确定要上传选中的图片吗？', function(r){
					if(r){
						...
						$('#rawPicUploadForm').form('submit', {
							url: "${pageContext.servletContext.contextPath}/ajuan/lsRawPicUpload.do",
							onSubmit: function(param){
								param.userId="${sessionScope.loginInfo.userId}";
								param.isls = "${isls}";
								param.aj_ewm = "${JadInfo.aj_ewm}";
								param.mldm = "${JadInfo.mldm}";
								param.currNode_id = "${currNode_id}";
								param.upload4Zl = "true";
							},
							success: function(data){
								...
								if(data == "" || data.indexOf("Exception") >= 0){
									...
									if (msgInfo != null && msgInfo != "null" && msgInfo.length > 0){//存在上传失败的文件
										...
										$.messager.alert('','上传成功',"",function(){
											$('#ajzlTree').tree('reload');//刷新树目录
											$('#lsajzlTree').tree('reload');//刷新树目录
											$("#ajzlPicUploadDilog").dialog("close");
										});
									}
								}
							}
...
// 			}
			...
			$("#tipInfo").textbox({
				...
				multiline : true,
				height : 90,
				value : "  提示：选择一个或多个卷宗影像图片上传到服务器。上传前请检查文件顺序与命名是否准确。",
				readonly : true
			});
			//文件选择框
			$("#file_upload").filebox({    
