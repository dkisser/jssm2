...
public class BrowserInterceptor implements HandlerInterceptor {
	...
	static {
		...
		try {
			...
			if (StringUtils.isEmpty(browserName)) {
				throw new RuntimeException("浏览器名称为空");
			}
			if (StringUtils.isEmpty(versionStr)) {
				throw new RuntimeException("浏览器版本号为空");
			}
		} catch (Exception e1) {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		//pageoffice和哲林高拍仪专有 
		if(browserInfo.contains("zspoupload") || browserInfo.contains("python-httplib")){
			return true;
		}
